...
function App() {
  ...
  const defaultLocation = { lat: 35.6580, lng: 139.7016 };
      ...
      if (position && !initialDataLoadedRef.current) {
        ...
        try {
          ...
          const allRestaurantsPromises = keywords.map(keyword => 
            fetchRestaurantsByKeyword(keyword, position.lat, position.lng, 5)
          );
  ...
  const handleKeywordClick = useCallback(async (keyword: string) => {
    ...
    const existingRestaurants = restaurants.filter(r => r.trendKeyword === keyword);
    ...
    try {
      ...
      if (realRestaurants.length > 0) {
        ...
        const newRestaurants = [...filteredRestaurants, ...realRestaurants];
  ...
  const handleDragMove = (e: React.MouseEvent | React.TouchEvent) => {
    ...
    const newHeight = Math.min(Math.max(mapHeight + deltaPercent, 20), 90);
  ...
  const exploreTabContent = (
    <div className="flex flex-col h-full relative">
      ...
      <div className={`relative z-10 w-full transition-opacity duration-100 ${tabChangeAnimation ? 'opacity-0 transform -translate-y-4' : 'opacity-100 transform translate-y-0'}`}>
        ...
        <div className="bg-white">
          {error && !position && (
            <div className="bg-red-100 border-l-4 border-red-500 text-red-700 p-4 mb-4 rounded mx-2">
              ...
              <p className="mt-2">代わりにデフォルトの位置を使用します。</p>
            </div>
          )}
  ...
  const appContent = (
    <div className="flex flex-col h-full relative">
      ...
      <div className="flex-1 overflow-hidden">
        {activeTab === 'explore' ? exploreTabContent : trendingTabContent}
      </div>
