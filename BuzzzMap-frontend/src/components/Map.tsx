...
const Map: React.FC<MapProps> = ({ 
  ...
  const [summarizedNames, setSummarizedNames] = useState<Record<string, string>>({});
...
const Map: React.FC<MapProps> = ({ 
        ...
        const mapOptions: MapOptions = {
          ...
          styles: [
            ...
            },
            {
              ...
              elementType: "geometry.fill",
              stylers: [{ color: "#dcebd7" }, { visibility: "on" }]
            },
            {
  ...
  useEffect(() => {
    const summarizeNames = async () => {
      const newSummarizedNames: Record<string, string> = { ...summarizedNames };
      const namesToSummarize = restaurants.filter(r => 
        r.name.length > 8 && !summarizedNames[r.id] && !nameCache[r.id]
      );
      ...
      for (const restaurant of namesToSummarize) {
        ...
        if (nameCache[restaurant.id]) {
          newSummarizedNames[restaurant.id] = nameCache[restaurant.id];
          continue;
        }
        
        try {
          const shortName = await summarizeRestaurantName(restaurant.name);
          ...
          newSummarizedNames[restaurant.id] = restaurant.name.substring(0, 7) + '…';
          nameCache[restaurant.id] = newSummarizedNames[restaurant.id]; // Add to global cache
        }
      }
  ...
  useEffect(() => {
    ...
    restaurants.forEach((restaurant, index) => {
      ...
      if (markersRef.current[restaurant.id]) {
        ...
        updateMarkerIcon(markersRef.current[restaurant.id], restaurant, activeKeyword, index, showCategoryLabels);
      } else {
        ...
        updateMarkerIcon(marker, restaurant, activeKeyword, index, showCategoryLabels);
        ...
        marker.addListener('click', () => {
          ...
          if (infoWindow) {
                ...
                <div class="flex items-center mt-1">
                  <div class="flex items-center">
                    ${Array(5).fill(0).map((_, i) => 
                      `<svg class="w-3 h-3 ${i < restaurant.rating ? 'text-yellow-400 fill-yellow-400' : 'text-gray-300'}" 
                  ...
                  <div class="flex items-center ml-2 text-gray-600">
                    ${Array(restaurant.priceLevel).fill(0).map(() => 
            ...
            setTimeout(() => {
              const detailButton = content.querySelector('button');
              if (detailButton) {
    ...
    if (Object.keys(markersRef.current).length > 0 && restaurants.length > 0) {
  ...
  }, [map, restaurants, infoWindow, onMarkerClick, activeKeyword, showCategoryLabels, summarizedNames]);
  ...
  const updateMarkerIcon = (
    ...
    if (activeKeyword && activeKeyword !== 'all') {
      ...
      const svgUrl = 'data:image/svg+xml;charset=UTF-8,' + encodeURIComponent(pinSVG);
      ...
      marker.setLabel({
        ...
        fontSize: '10px',
        fontWeight: 'bold',
        className: `marker-label marker-label-${index % 2} marker-label-transparent`
      });
      ...
      // Convert SVG to data URL
      const categoryPinUrl = 'data:image/svg+xml;charset=UTF-8,' + encodeURIComponent(categoryPinSVG);
