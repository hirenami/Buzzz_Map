...
export const fetchRestaurantsByLocation = async (
	...
	try {
		...
		const url = `https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=${lat},${lng}&radius=1500&type=restaurant&keyword=${encodeURIComponent(keyword)}&language=ja&key=${VITE_API_KEY}`;
...
function mapGooglePlacesToRestaurants(
	...
	return places.slice(0, limit).map(place => {
		...
		const photoUrl = place.photos && place.photos.length > 0
			? `https://maps.googleapis.com/maps/api/place/photo?maxwidth=400&photoreference=${place.photos[0].photo_reference}&key=${VITE_API_KEY}`
