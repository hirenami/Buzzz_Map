...
type TrendinglongSearch struct {
...
func (a *Api) CallPerplexityLongAPI(query []RisingSearch) ([]TrendinglongSearch, error) {
	// Perplexity APIのURL
	url := "https://api.perplexity.ai/chat/completions" // Perplexity APIエンドポイント

	var items []TrendinglongSearch
	...
	for _, q := range query {
		...
		var search TrendinglongSearch

		// "飲食店名"の場合
		if content == "飲食店名" {
			search.Query = q.Query
			search.IncreasePercentage = q.ExtractedValue
			search.Months = q.Months
		} else if content == "食品名" || content == "食材名" {
			// "食品名"または"食材名"の場合、場所を生成するためのプロンプトを作成
			locationPrompt := fmt.Sprintf("次のアイテムはどこで売られていますか？最も売られている可能性が高い場所の単語を一つ出力してください。地名ではなく、店でお願いします。その単語以外は絶対に出力しないでください。: %s", q.Query)
			...
			search.IncreasePercentage = q.ExtractedValue
			search.Months = q.Months
			search.Location = locationResponse.Choices[0].Message.Content
		}
